...
export default function ServiceModal({ isOpen, onClose, service }: ServiceModalProps) {
    ...
    // Trigger CSS enter/exit
    useEffect(() => {
        if (isOpen) {
            // Next tick so initial state is applied first
    ...
    const modalContent = (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-6">
            ...
            <div
                onClick={onClose}
                className="absolute inset-0 bg-brand-indigo/30 backdrop-blur-xl transition-opacity duration-300"
                style={{ opacity: visible ? 1 : 0 }}
            />

            {/* Modal Content */}
            <div
                className="relative w-full max-w-3xl max-h-[90vh] bg-white/95 backdrop-blur-2xl rounded-3xl shadow-[0_30px_100px_-20px_rgba(28,31,58,0.15)] overflow-hidden flex flex-col border border-white/50 transition-all duration-300"
                style={{
                    opacity: visible ? 1 : 0,
